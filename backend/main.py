...
@app.route('/users')
def get_users():
    return jsonify([{"name": "John"}, {"name": "Doe"}])
...
@app.route("/article")
def article():
    return render_template('article.js')
...
#이런식으로 react에서는 registeration.js에서 페이지를 redirect! redirect(url_for())사용안함 근데 로그인이 안됨ㅠㅠ 확인필요
...
def register():
    ...
    if user:
        return jsonify({"message": "You already have ID. Go to Login."}), 400
    else:
...
def project():    
    ...
    if request.method == "POST" :
        ...
        db.session.commit()
        projects = Project.query.filter_by(user_id=current_user.id).all()
        return render_template('project.js', projects = projects)
    return render_template('project.js', projects = projects)
...
def file(project_number):
    ...
    if request.method == "POST":
        ...
        db.session.commit()
        files = File.query.filter_by(project_id=project_number).all()
        return render_template('file.js', projects=projects, files=files, project_id=project_number)
    return render_template('file.js', projects=projects, files=files, project_id=project_number)
...
def processing(project_id, file_id):
    ...
    return render_template('processing.js', projects=projects, file=file, images=original_images,
